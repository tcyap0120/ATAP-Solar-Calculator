...
const CalculatorPage: React.FC = () => {
  ...
  useEffect(() => {
      ...
      if (inverterSize === '0' && meters.length > 0 && selectedMeterId) {
          ...
          if (m && m.maxInverterKw > 0) {
               setInverterSize(m.maxInverterKw.toString());
          }
      }
  ...
  useEffect(() => {
     if ((!billAmount || billAmount === '0') && !hasDefaultedMeter) {
        ...
        if (meter && meter.maxInverterKw > 0) {
            const newInverterKw = meter.maxInverterKw;
            setInverterSize(newInverterKw.toString());
            
            const oversizing = settings.oversizingRatio || 1.35;
        ...
        } else if (meter && meter.id === 'tbc') {
            setInverterSize('0');
            setHasDefaultedMeter(true);
        }
     }
  }, [selectedMeterId, meters, settings.panelRating, billAmount, hasDefaultedMeter, roofLimit, settings.oversizingRatio]);
  ...
  const handleMeterChange = (newMeterId: string) => {
      ...
      if (meter) {
          setInverterSize(meter.maxInverterKw.toString());
          // Auto-Recalc Capacity if not in "Bill Mode"
          if (!billAmount || billAmount === '0') {
             if (meter.id === 'tbc') {
                 // Do nothing / keep existing values if TBC
             } else {
                const oversizing = settings.oversizingRatio || 1.35;
                const maxDc = meter.maxInverterKw * oversizing;
                let maxPanels = Math.floor(maxDc / settings.panelRating);
                if (roofLimit && parseFloat(roofLimit) > 0) {
  ...
  }, [kwpValue, selectedBrandId, selectedMeterId, settings.oversizingRatio]);
  ...
  const handleRecommend = () => {
    ...
    let maxAllowedPanelsByInverter = Infinity;
    const oversizing = settings.oversizingRatio || 1.35;
    ...
    if (selectedMeter && selectedMeter.limitKwac < 90000) {
        ...
        const maxDcLimit = constraintKw * oversizing; 
        maxAllowedPanelsByInverter = Math.floor(maxDcLimit / settings.panelRating);
    } 

    const roofMaxPanels = roofLimit && parseFloat(roofLimit) > 0 ? parseFloat(roofLimit) : Infinity;
    const absoluteMaxPanels = Math.min(maxAllowedPanelsByInverter, roofMaxPanels);
    ...
    const calcPlanDetails = (kwp: number, battCount: number = 0) => {
        ...
        if (battCount > 0 && settings.battery) {
            cost += (battCount * settings.battery.capacityKwh * settings.battery.pricePerKwh);
        }
    ...
    const finalPanelsA = Math.min(requiredPanelsA, absoluteMaxPanels);
    const planA = calcPlanDetails(finalPanelsA * settings.panelRating);
    ...
    const finalPanelsB = Math.min(requiredPanelsB, absoluteMaxPanels);
    const planB = calcPlanDetails(finalPanelsB * settings.panelRating);

    // PLAN C: BESS COVERAGE
    const battSettings = settings.battery || { capacityKwh: 16, usableRatio: 0.9, pricePerKwh: 800, maxCount: 4 };
    ...
    const finalPanelsC = Math.min(requiredPanelsC, absoluteMaxPanels);
    ...
    applyPlan(plans[1]); // Default to Daytime Coverage
  };
  ...
  const applyPlan = (plan: ProposedPlan) => {
    ...
    // Inverter size is now handled by the useEffect watching kwpValue and brand
  }
  ...
  const maxSafePanels = useMemo(() => {
      if (!selectedMeter) return Infinity;
      if (selectedMeter.id === 'tbc') return Infinity;
      
      const oversizing = settings.oversizingRatio || 1.35;
      const maxDc = selectedMeter.maxInverterKw * oversizing;
      return Math.floor(maxDc / settings.panelRating);
  }, [selectedMeter, settings.oversizingRatio, settings.panelRating]);
  ...
  const calculateResult = (): DetailedResult | null => {
    ...
    if (batteryCount > 0 && settings.battery) {
        ...
        battCost = battTotalCap * settings.battery.pricePerKwh;
        price += battCost;
    }
    ...
    if (maintenanceTiers) {
        ...
        if (maintTier) {
            const freq = maintTier.frequencyYears || 1;
            maintCost = maintTier.cost / freq;
        }
    }
    ...
    if (batteryCount > 0 && settings.battery) {
        ...
        if (postSolarDuration > 0) {
            const consumptionPostSolarRatio = postSolarDuration / opDuration;
            const consumptionPostSolar = dailyActiveConsumption * consumptionPostSolarRatio;
            dailyBatteryDischarge = Math.min(dailyBatteryCharge, consumptionPostSolar);
        }
    }
    ...
    return {
      ...
      breakdownOpex: {
          ...
          maintenanceDetails: maintTier ? { cost: maintTier.cost, freq: maintTier.frequencyYears } : undefined
      },
      isGitaEligible,
      batteryCount,
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                     ...
                     <div className="space-y-4 border-r border-slate-100 pr-4">
                         <div>
                             ...
                             {result.monthlyAtapValue > 1 && (
                                <p className="text-[10px] font-bold text-amber-600 mt-1">
                                    + RM {result.monthlyAtapValue.toLocaleString(undefined, { maximumFractionDigits: 0 })} (Excess)
                                </p>
                             )}
                         </div>
                         ...
                         <div>
                             ...
                             <p className="text-[10px] text-slate-400">Direct Use {result.batteryDischargeKwh > 0 ? '+ Battery ' : ''}+ Usable Export</p>
                         </div>
                     </div>
                                  ...
                                  <div className="grid grid-cols-2">
                                      ...
                                      <div className="p-3 border-r border-slate-100 bg-blue-50/30">
                                          ...
                                          <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                              RM {(result.monthlyDirectUseKwh * result.effectiveTariff).toLocaleString(undefined, {maximumFractionDigits:0})} Saved
                                          </div>
                                      </div>
                                      ...
                                      <div className="p-3 border-r border-t border-slate-100 bg-purple-50/30">
                                          <div className="flex items-center gap-1.5 mb-1">
                                             ...
                                             <span className="text-[10px] font-bold text-purple-600 uppercase">Usable Export</span>
                                          </div>
                                          <div className="text-sm font-bold text-slate-800">
                                              {result.monthlyUsableExportKwh.toLocaleString(undefined, {maximumFractionDigits:0})} kWh
                                          </div>
                                          <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                              RM {(result.monthlyUsableExportKwh * result.exportRate).toLocaleString(undefined, {maximumFractionDigits:0})} Credit
                                          </div>
                                      </div>
                                      ...
                                      <div className="p-3 border-t border-slate-100 bg-amber-50/30">
                                          <div className="flex items-center gap-1.5 mb-1">
                                             ...
                                             <span className="text-[10px] font-bold text-amber-600 uppercase">Excess (ATAP)</span>
                                          </div>
                                          <div className="text-sm font-bold text-slate-800">
                                              {result.monthlyAtapKwh.toLocaleString(undefined, {maximumFractionDigits:0})} kWh
                                          </div>
                                          <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                              ~ RM {result.monthlyAtapValue.toLocaleString(undefined, {maximumFractionDigits:0})} Value
                                          </div>
                                      </div>
             ...
             <div className="bg-slate-50 rounded-[2rem] shadow-sm border border-slate-200 p-6 animate-fade-in-up delay-100">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    {result.breakdownOpex.maintenance > 0 && (
                        <div className="bg-white p-3 rounded-xl border border-slate-200" title="Based on system size tier">
                            <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Maintenance</p>
                            <p className="font-bold text-slate-700 mb-1">RM {result.breakdownOpex.maintenance.toLocaleString(undefined, {maximumFractionDigits:0})}<span className="text-[10px] font-normal text-slate-400">/yr</span></p>
                            <span className="bg-emerald-100 text-emerald-700 text-[9px] font-bold px-1.5 py-0.5 rounded">Recommended</span>
                        </div>
                    )}
