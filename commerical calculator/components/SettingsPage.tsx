...
import { DEFAULT_BRANDS, DEFAULT_METERS, DEFAULT_SETTINGS } from '../constants';
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-8 min-h-[400px]">
        ...
        {activeTab === 'meters' && (
            <div className="space-y-6">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                     <div>
                        <h3 className="font-bold text-lg text-slate-800">Meter Configurations</h3>
                        <p className="text-sm text-slate-500">Define AC limits and max inverter sizes per meter type.</p>
                     </div>
                     <div>
                         <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Oversizing Ratio (DC/AC)</label>
                         <input 
                            type="number" step="0.01"
                            value={localSettings.oversizingRatio || 1.35}
                            onChange={(e) => setLocalSettings({...localSettings, oversizingRatio: parseFloat(e.target.value)})}
                            className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-bold w-32 text-right focus:ring-2 focus:ring-amber-500 outline-none"
                         />
                     </div>
                </div>
                ...
                <div className="overflow-hidden border border-slate-200 rounded-2xl shadow-sm">
                     <table className="w-full text-sm text-left min-w-[600px]">
                        <thead className="text-xs font-bold text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                            <tr>
                                <th className="px-6 py-4">Meter Type</th>
                                <th className="px-6 py-4">AC Limit (kW)</th>
                                <th className="px-6 py-4">Max Inverter (kW)</th>
                                <th className="px-6 py-4">Max Panels (Est.)</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 bg-white">
                            {meters.map(meter => (
                                <tr key={meter.id} className="hover:bg-slate-50/50">
                                    <td className="px-6 py-4 font-bold text-slate-700">{meter.name}</td>
                                    ...
                                    <td className="px-6 py-4">
                                        <input 
                                            type="number" step="1"
                                            className="bg-slate-50 border border-slate-200 focus:border-amber-400 rounded-lg px-3 py-2 w-32 font-medium outline-none transition-colors"
                                            value={meter.maxInverterKw || 0}
                                            onChange={(e) => updateMeterField(meter.id, 'maxInverterKw', e.target.value)}
                                        />
                                    </td>
                                    <td className="px-6 py-4 text-slate-500 font-bold">
                                        {meter.maxInverterKw > 0 
                                            ? Math.floor((meter.maxInverterKw * (localSettings.oversizingRatio || 1.35)) / localSettings.panelRating)
                                            : '-'}
                                    </td>
                                </tr>
                            ))}
            ...
            <div className="space-y-6">
                 <div className="bg-blue-50 border border-blue-100 p-5 rounded-2xl flex gap-4 items-start">
                    ...
                    <div className="text-sm text-blue-900">
                        ...
                        <p className="opacity-80 leading-relaxed">This lookup table is used when "Smart Logic" is enabled for a pricing tier. It matches the exact number of panels to base prices.</p>
                    </div>
                 </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs font-bold text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="px-6 py-4">Auto-Select Brand</th>
                                <th className="px-6 py-4"></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 bg-white">
                            {(localSettings.brandRules || []).map((rule, idx) => (
                                <tr key={idx} className="hover:bg-slate-50/50">
                                    <td className="px-6 py-4">
                                        <input 
                                            type="number"
                                            className="w-24 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 font-medium"
                                            value={rule.minKwp}
                                    ...
                                    <td className="px-6 py-4">
                                        <select 
                                            className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 font-medium outline-none focus:ring-2 focus:ring-purple-200"
                                            value={rule.brandId}
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => removeBrandRule(idx)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                                            <Trash2 size={16} />
                                        </button>
                                    </td>
                                </tr>
        ...
        {activeTab === 'other_costs' && (
            <div className="space-y-12">
                ...
                {/* 2. Maintenance Tiers */}
                <div>
                    <h3 className="font-bold text-xl text-slate-800 mb-4">Maintenance Cost Tiers</h3>
                    <div className="overflow-hidden border border-slate-200 rounded-2xl shadow-sm">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs font-bold text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                                <tr>
        ...
        {activeTab === 'battery' && (
            <div className="space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl">
                     ...
                     <div className="group">
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide mb-2 group-focus-within:text-emerald-500 transition-colors">Price per Capacity (RM/kWh)</label>
                        <input 
                            type="number" step="1"
                            value={localSettings.battery?.pricePerKwh || 800}
                            onChange={e => updateBattery('pricePerKwh', e.target.value)}
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                        />
                     </div>
                     <div className="group">
