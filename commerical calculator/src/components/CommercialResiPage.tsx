...
const CommercialResiPage: React.FC = () => {
  ...
  const [roofLimit, setRoofLimit] = useState<number>(60);
  ...
  const [batteryComparisonMode, setBatteryComparisonMode] = useState<boolean>(false);

  const [expandedCardId, setExpandedCardId] = useState<string | null>(null);

  // --- Default Configs ---
  const defaultCardConfigs: PlanConfig[] = [
    { id: 'fastest', label: 'Quickest ROI', panels: 14, batteryQty: 0, isSelected: true, minSavings: 0, maxSavings: 100, colorClass: 'border-emerald-500 bg-emerald-50/50', badge: 'Best Value' },
    { id: '60-70', label: 'Balanced Saver', panels: 21, batteryQty: 0, isSelected: false, minSavings: 60, maxSavings: 70, colorClass: 'border-blue-500 bg-blue-50/50', badge: 'Popular' },
    { id: '70-80', label: 'High Offset', panels: 30, batteryQty: 0, isSelected: false, minSavings: 70, maxSavings: 80, colorClass: 'border-indigo-500 bg-indigo-50/50', badge: 'Max Solar' },
    { id: '80-90', label: 'Hybrid Entry', panels: 30, batteryQty: 1, isSelected: false, minSavings: 80, maxSavings: 90, colorClass: 'border-violet-500 bg-violet-50/50', badge: 'Hybrid' },
    { id: '90-95', label: 'Grid Independence', panels: 30, batteryQty: 3, isSelected: true, minSavings: 90, maxSavings: 95, colorClass: 'border-cyan-500 bg-cyan-50/50', badge: 'Premium' },
    { id: '95-100', label: 'Total Coverage', panels: 30, batteryQty: 2, isSelected: false, minSavings: 95, maxSavings: 100, colorClass: 'border-emerald-500 bg-emerald-50/50', badge: 'Ultimate' },
  ];

  const [cardConfigs, setCardConfigs] = useState<PlanConfig[]>(defaultCardConfigs);
  ...
  const calculateMetrics = (pQty: number, bQty: number, tKwh: number) => {
    ...
    // 5. Battery Logic
    // FIXED: Use settings capacity (default 14.3 or user set) instead of hardcoded 5.0
    const kwhPerBatteryUnit = settings.battery?.capacityKwh || 5.0; 
    ...
    // 9. Pricing Logic
    let baseRefPrice = 0;
    let baseRefPriceCC = 0;
    
    const ref = settings.referencePrices?.find(r => r.panels === pQty);
    
    // Fallback if no ref price found (should not happen with standard config, but good safety)
    const fallbackBase = solarSize * 2500;

    if (ref) {
        baseRefPrice = ref.price;
        // Use PriceCC36 if available, else estimate
        baseRefPriceCC = ref.priceCC36 ?? (ref.price * 1.093); 
    } else {
        baseRefPrice = fallbackBase;
        baseRefPriceCC = fallbackBase * 1.093;
    }
    
    // Inverter Logic: Add RM3000 ALWAYS
    const inverterPremium = 3000; 
    
    // Battery Cost: 
    // Cash = RM 7400
    // CC/Installment = RM 8000
    const batteryCostCash = bQty * 7400;
    const batteryCostCC = bQty * 8000;
    
    const totalPrice = baseRefPrice + inverterPremium + batteryCostCash;
    const installmentTotal = baseRefPriceCC + inverterPremium + batteryCostCC;
  ...
  }, [manualPanels, manualBattery, usageKwh, daytimePercent, settings]);

  // --- Auto-Optimizer Logic ---
  useEffect(() => {
    if (!showRecommendations) return; 

    // Battery Comparison Mode Logic
    if (batteryComparisonMode) {
        const panels = Math.min(roofLimit, 60); // Cap at 60 or roof limit
        const batteryOptions = [0, 1, 2, 3, 4];
        
        const comparisonConfigs: PlanConfig[] = batteryOptions.map(bQty => ({
            id: `batt-comp-${bQty}`,
            label: bQty === 0 ? 'Max Solar Only' : `Max Solar + ${bQty} Batt`,
            panels: panels,
            batteryQty: bQty,
            isSelected: bQty === 1, // Select 1 batt by default for interest
            minSavings: 0,
            maxSavings: 100,
            colorClass: bQty === 0 ? 'border-amber-500 bg-amber-50/50' : 'border-purple-500 bg-purple-50/50',
            badge: bQty === 0 ? 'Base' : `${bQty} Battery`,
            isBatteryCompare: true
        }));
        setCardConfigs(comparisonConfigs);
        return;
    }

    // Standard Optimizer Logic
    const newConfigs = defaultCardConfigs.map(config => {
      let bestP = config.panels;
      let bestB = config.batteryQty;
      let minRoi = 99;
      let foundViable = false;

      // Scan for best fit in bracket
      for (let p = 6; p <= Math.min(roofLimit, 60); p++) {
        for (let b = 0; b <= 4; b++) {
          const m = calculateMetrics(p, b, usageKwh);
          
          // CRITICAL: Filter out High Export scenarios for recommendations
          if (m.isHighExport) continue;

          if (config.id === 'fastest') {
             if (b === 0 && m.roi < minRoi && m.roi > 0) {
               minRoi = m.roi; bestP = p; bestB = b; foundViable = true;
             }
          } else {
             if (m.savingsPct >= config.minSavings && m.savingsPct < config.maxSavings) {
                 if (m.roi < minRoi && m.roi > 0) {
                    minRoi = m.roi; bestP = p; bestB = b; foundViable = true;
                 }
             }
          }
      ...
      return { ...config, panels: bestP, batteryQty: bestB, isViable: foundViable };
    });
    
    setCardConfigs(newConfigs);

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [usageKwh, daytimePercent, roofLimit, showRecommendations, batteryComparisonMode]);

  // --- Handlers ---
  const updateCard = (id: string, field: 'panels' | 'batteryQty' | 'isSelected', delta: number | boolean) => {
    setCardConfigs(prev => prev.map(c => {
      if (c.id !== id) return c;
      if (field === 'isSelected') return { ...c, isSelected: delta as boolean };
      const currentVal = c[field] as number;
      const newVal = Math.max(0, currentVal + (delta as number));
  ...
  const selectedPlansForWA = useMemo(() => {
    ...
    if (!showRecommendations) {
        ...
        const recommended = cardConfigs.filter(c => c.isSelected).map(c => {
            const m = calculateMetrics(c.panels, c.batteryQty, usageKwh);
            return { ...c, ...m };
        });
        plans.push(...recommended);
    }
    return plans;
  }, [cardConfigs, usageKwh, daytimePercent, manualMetrics, manualPanels, manualBattery, showRecommendations]);
  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-24">
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-6 md:p-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            <div>
                ...
                <p className="text-slate-500 font-medium text-sm mt-1">Commercial - Residential Style Usage Estimation</p>
            </div>
             ...
             <div className="space-y-3">
                ...
                <div className="relative group">
                    <input 
                        type="number" 
                        value={roofLimit}
                        onChange={(e) => setRoofLimit(Math.max(0, parseInt(e.target.value) || 0))}
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-2xl font-black text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all group-hover:bg-slate-100"
                    />
                 ...
                 <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-4">
                     ...
                     <div className="flex items-end gap-2">
                      ...
                      <p className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-lg inline-block">
                         {(manualBattery * (settings.battery?.capacityKwh || 5)).toFixed(1)} kWh Storage
                     </p>
                 </div>
             </div>
                 ...
                 <div className="bg-slate-900 rounded-[2rem] p-8 text-white h-full relative overflow-hidden flex flex-col justify-between shadow-2xl shadow-slate-900/10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10 mb-8">
                         ...
                         <div>
                             ...
                             <h3 className="text-4xl font-black text-white tracking-tight">RM {manualMetrics.totalPrice.toLocaleString(undefined, {maximumFractionDigits:0, notation: "compact"})}</h3>
                    ...
                    <div className="bg-white/10 rounded-2xl p-6 backdrop-blur-sm border border-white/10 relative z-10">
                         ...
                         <div className="flex items-center gap-4 text-sm mb-4">
                             <div className="flex-1">
                                 ...
                                 <p className="text-xl font-bold text-slate-300 line-through decoration-red-500/50">RM {manualMetrics.originalGrandTotal.toFixed(0)}</p>
                             </div>
                             <ArrowRight size={20} className="text-slate-500" />
                             <div className="flex-1 text-right">
                                 <p className="text-emerald-400 text-xs mb-1 font-bold">New Estimate</p>
                                 <p className="text-3xl font-black text-white">RM {manualMetrics.newGrandTotal.toFixed(0)}</p>
                             </div>
                         </div>
      ...
      <div className="flex flex-col items-center justify-center py-8 gap-4 border-t border-slate-200 mt-8">
          <div className="text-center">
              <h3 className="font-bold text-slate-700">Not sure what to choose?</h3>
              <p className="text-sm text-slate-500">Let our AI analyze your consumption and recommend optimal configurations.</p>
          </div>
          ...
          <div className="flex gap-4">
              <button 
                onClick={() => {
                    ...
                    if(!showRecommendations) setBatteryComparisonMode(false); // Default to standard
                }}
                className={`px-8 py-3 rounded-2xl font-black flex items-center gap-2 transition-all ${showRecommendations && !batteryComparisonMode ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
              >
                  {showRecommendations && !batteryComparisonMode ? (
                      <>Hide Recommendations <ChevronUp size={18}/></>
                  ) : (
                      <><LayoutGrid size={18}/> View Auto-Recommendations</>
                  )}
              </button>
              ...
              <button 
                onClick={() => {
                    ...
                    setBatteryComparisonMode(!batteryComparisonMode);
                }}
                className={`px-6 py-3 rounded-2xl font-black flex items-center gap-2 transition-all ${batteryComparisonMode ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/20' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
              >
                  <BarChart3 size={18}/> {batteryComparisonMode ? 'Comparing Batteries...' : 'Compare Batteries'}
              </button>
          </div>
      </div>

      {/* --- CARDS GRID (CONDITIONAL) --- */}
      {showRecommendations && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in-up">
            ...
            return (
                ...
                >
                {/* Warning Overlay for Battery Compare Mode */}
                {metrics.isHighExport && config.isBatteryCompare && (
                     <div className="bg-amber-100 text-amber-800 text-[10px] font-bold text-center py-1 flex items-center justify-center gap-1">
                         <AlertTriangle size={10} /> High Export
                     </div>
                )}
                ...
                <div className="p-6 pb-2">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md mb-2 inline-block ${isSelected ? 'bg-white/40 text-slate-700' : 'bg-slate-100 text-slate-500'}`}>
                                {config.badge}
                            </span>
                            <h3 className="text-xl font-extrabold text-slate-800 leading-none">{config.label}</h3>
                        </div>
                        <button 
                            onClick={() => updateCard(config.id, 'isSelected', !isSelected)}
                            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isSelected ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-300 hover:bg-slate-200'}`}
                        >
                            {isSelected ? <CheckCircle2 size={20} /> : <div className="w-5 h-5 rounded-full border-2 border-slate-300"></div>}
                        </button>
                    </div>

                    {/* Config Controls */}
                    <div className="flex gap-2 mb-6">
                        <div className={`flex-1 flex items-center justify-between p-2 rounded-xl border ${isSelected ? 'bg-white/40 border-white/20' : 'bg-slate-50 border-slate-100'}`}>
                            ...
                            <div className="flex flex-col gap-1">
                                <button onClick={() => updateCard(config.id, 'panels', 1)} className="p-0.5 hover:bg-black/5 rounded"><ChevronUp size={12}/></button>
                                <button onClick={() => updateCard(config.id, 'panels', -1)} className="p-0.5 hover:bg-black/5 rounded"><ChevronDown size={12}/></button>
                            </div>
                        </div>
                        ...
                        <div className={`flex-1 flex items-center justify-between p-2 rounded-xl border ${isSelected ? 'bg-white/40 border-white/20' : 'bg-slate-50 border-slate-100'}`}>
                            ...
                            <div className="flex flex-col gap-1">
                                <button onClick={() => updateCard(config.id, 'batteryQty', 1)} className="p-0.5 hover:bg-black/5 rounded"><ChevronUp size={12}/></button>
                                <button onClick={() => updateCard(config.id, 'batteryQty', -1)} className="p-0.5 hover:bg-black/5 rounded"><ChevronDown size={12}/></button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 rounded-lg p-2 px-3 border border-slate-100">
                             ...
                             <span className="text-lg font-black text-slate-800">RM {metrics.totalPrice.toLocaleString(undefined, {notation: "compact"})}</span>
                        </div>

                        <div className="flex justify-between items-center">
                            <span className="text-xs font-bold text-slate-500 uppercase">Monthly Savings</span>
                            <span className="text-2xl font-black text-slate-800 tracking-tight">RM {metrics.monthlySavings.toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                        </div>
                        <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                            <div className="bg-emerald-500 h-full rounded-full" style={{width: `${Math.min(100, metrics.savingsPct)}%`}}></div>
                        </div>
                ...
                {/* Detail Expander */}
                <div className={`border-t ${isSelected ? 'border-black/5' : 'border-slate-100'} bg-white/30`}>
                    <button 
                        onClick={() => setExpandedCardId(isExpanded ? null : config.id)}
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm whitespace-pre-wrap font-sans text-xs sm:text-sm text-slate-700 leading-relaxed max-h-[500px] overflow-y-auto font-medium">
                              ...
                              {(() => {
                                  ...
                                  const roofText = roofLimit ? `${roofLimit} ${proposalLang === 'zh' ? '片' : 'pcs'}` : (proposalLang === 'zh' ? "待确认" : "TBC");
                               ...
                               <div ref={proposalRef} className="bg-white min-w-[700px] pb-4">
                                   ...
                                   <div className="bg-gradient-to-r from-violet-600 to-indigo-700 text-white p-6">
                                       ...
                                       <div className="text-xs text-indigo-100 font-medium flex gap-3 opacity-90">
                                           ...
                                           <span>•</span>
                                           <span>{proposalLang === 'zh' ? '屋顶限制' : 'Roof'}: {roofLimit || (proposalLang === 'zh' ? '待确认' : 'TBC')}</span>
                                       </div>
                                   </div>
