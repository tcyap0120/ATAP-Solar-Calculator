...
import { Calculator, Settings, Sun, FileText, Home } from 'lucide-react';
...
import {
  COMMERCIAL_METERS_SCHEMA_VERSION,
  COMMERCIAL_SETTINGS_SCHEMA_VERSION,
  DEFAULT_BRANDS,
  DEFAULT_METERS,
  DEFAULT_SETTINGS,
  DEFAULT_SESSION,
  commercialReferencePricesFromResidential,
  sanitizeCommercialReferencePrices,
  sanitizeGoodWeHybridRefRateTier,
} from './constants';
import { isSupportedCommercialMeterList } from './utils/meterHelpers';

/** Isolated localStorage keys so commercial settings never clash with residential app keys. */
const LS_SETTINGS = 'atap_commercial_settings';
const LS_BRANDS = 'atap_commercial_brands';
const LS_METERS = 'atap_commercial_meters';
const LS_METERS_SCHEMA = 'atap_commercial_meters_schema';
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [settings, setSettingsState] = useState<GlobalSettings>(() => {
    ...
    // Version must come from saved JSON only — merged picks DEFAULT's version when key is absent and skips migration while old brandRules still win.
    const savedSchema = parsed._settingsSchemaVersion ?? 0;
    const merged: GlobalSettings = { ...DEFAULT_SETTINGS, ...parsed };
    if (savedSchema < COMMERCIAL_SETTINGS_SCHEMA_VERSION) {
      merged.brandRules = [...DEFAULT_SETTINGS.brandRules];
      if (savedSchema < 4) {
        merged.panelRating = DEFAULT_SETTINGS.panelRating;
        merged.battery = { ...DEFAULT_SETTINGS.battery };
      }
      if (savedSchema < 5) {
  ...
  const updateSettings = useCallback((newSettings: GlobalSettings) => {
    setSettingsState(sanitizeCommercialReferencePrices(newSettings));
  }, []);

  const [brands, setBrandsState] = useState<InverterBrand[]>(() => {
    const saved = localStorage.getItem(LS_BRANDS);
    if (!saved) return DEFAULT_BRANDS;
    try {
      const parsed = JSON.parse(saved) as InverterBrand[];
      if (!parsed.some((b) => b.id === 'goodwe')) {
        return DEFAULT_BRANDS;
      }
      return sanitizeGoodWeHybridRefRateTier(parsed);
    } catch {
      return DEFAULT_BRANDS;
    }
  });

  const updateBrands = useCallback((newBrands: InverterBrand[]) => {
    setBrandsState(sanitizeGoodWeHybridRefRateTier(newBrands));
  }, []);

  const [meters, setMeters] = useState<MeterType[]>(() => {
    const schemaVer = parseInt(localStorage.getItem(LS_METERS_SCHEMA) || '0', 10);
    if (schemaVer < COMMERCIAL_METERS_SCHEMA_VERSION) {
      localStorage.setItem(LS_METERS_SCHEMA, String(COMMERCIAL_METERS_SCHEMA_VERSION));
      return DEFAULT_METERS;
    }
    const saved = localStorage.getItem(LS_METERS);
    if (!saved) return DEFAULT_METERS;
    try {
      const parsed = JSON.parse(saved) as MeterType[];
      if (!isSupportedCommercialMeterList(parsed)) {
        return DEFAULT_METERS;
      }
      return parsed;
    } catch {
  ...
  useEffect(() => {
    localStorage.setItem(LS_METERS, JSON.stringify(meters));
  }, [meters]);
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return (
    <div className="min-h-screen bg-slate-50 pb-24 md:pb-0 md:pl-24 transition-all overflow-x-hidden">
      {/* top offset = residential "Back" bar: fixed nav must not use viewport top:0 when embedded in App.tsx */}
      <nav className="hidden md:flex flex-col items-center fixed left-0 top-[var(--atap-commercial-header-h,0px)] bottom-0 w-24 bg-white border-r border-slate-100 z-[90] shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
...
/** Commercial solar calculator (standalone UI + hash routes). Mount inside residential app only when selected. */
const CommercialSolarShell: React.FC = () => {
  return (
    <AppProvider>
      <HashRouter>
        <Layout>
...
export default CommercialSolarShell;
